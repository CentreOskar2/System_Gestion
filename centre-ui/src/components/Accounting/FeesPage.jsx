...
export default function FeesPage() {
  ...
  return <div className="fees-page"><Header /><main className="fees-content"><div className="fees-heading"><h1>Comptabilité</h1><p>Gestion financière du centre.</p></div><nav className="accounting-tabs"><Link className="active" to="/accounting/fees">Frais de scolarité</Link><Link to="/accounting/delinquencies">Retards & Impayés</Link><Link to="/accounting/salaries">Salaires Profs</Link><Link to="/accounting/expenses">Charges</Link><Link to="/accounting/profit">Bénéfice net</Link></nav>
    <section className="fee-stats"><article><span>Total encaissé</span><strong>160 800 DH</strong><i className="fee-stat-icon fee-stat-icon--green"><Icon name="trending-up" /></i></article><article><span>Élèves facturés</span><strong>{students.length}</strong><i className="fee-stat-icon"><Icon name="users" /></i></article><article><span>Dû mensuel total</span><strong>{students.reduce((sum,s) => sum + s.monthly, 0).toLocaleString('fr-FR')} DH</strong><i className="fee-stat-icon fee-stat-icon--red"><Icon name="wallet" /></i></article></section>

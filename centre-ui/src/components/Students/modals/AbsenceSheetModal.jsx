...
function AttendanceSheet({ values, close }) {
  ...
  return <main className="absence-sheet-page"><div className="absence-sheet-actions"><button onClick={close}>← Retour aux étudiants</button><button className="absence-print" disabled={isExporting} onClick={downloadPdf}>{isExporting ? 'Génération du PDF…' : '▣  Télécharger le PDF'}</button></div><article ref={sheetRef} className="absence-sheet"><header className="absence-sheet-header"><div className="absence-brand"><img src="/oskar-logo.png" alt="Centre Atlas" /><div><strong>Centre Atlas</strong><span>Fiche de présence —</span></div></div><div><strong>Fiche d'absence</strong><span>Année scolaire 2025 – 2026</span></div></header><section className="absence-details"><div><small>PROFESSEUR</small><b>{values.teacher}</b></div><div><small>MATIÈRE</small><b>{values.subject}</b></div><div><small>NIVEAU</small><b>{values.level}</b></div><div><small>GROUPE</small><b>{values.group}</b></div></section><table className="absence-table"><thead><tr><th>#</th><th>Élève</th>{sessions.map(session => <th key={session}>{session}</th>)}</tr></thead><tbody>{roster.map((name, index) => <tr key={name}><td>{index + 1}</td><td>{name}</td>{sessions.map(session => <td key={session} />)}</tr>)}</tbody></table><p className="absence-help">Cocher <b>P</b> pour présent, <b>A</b> pour absent, <b>R</b> pour retard.</p><footer className="absence-signatures"><span>Signature du professeur</span><span>Signature de l'administration</span></footer></article></main>
}

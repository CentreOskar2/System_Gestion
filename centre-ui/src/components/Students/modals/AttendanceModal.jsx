...
export default function AttendanceModal({ student, close }) {
  ...
  return (
    <div className="student-overlay" onMouseDown={close}>
      <section className="attendance-modal" onMouseDown={stopPropagation}>
        ...
        {attendanceItems.map((item) => (
          <label
            className={`attendance-item ${selected.includes(item.id) ? 'checked' : ''}`}
            key={item.id}
          >
            <span>
              <input
            ...
            {selected.includes(item.id) && item.detail &&
              (item.id === 'retard' ? (
                <label>
                  Nombre de minutes
                ...
                <textarea placeholder={item.detail} />
              ))}
          </label>
        ))}
        <footer>
